 ...
 */
#ifndef INDUSTRIAL_MOVEIT_STOMP_MOVEIT_INCLUDE_STOMP_MOVEIT_COST_FUNCTIONS_TOOL_PATH_POSE_H_
#define INDUSTRIAL_MOVEIT_STOMP_MOVEIT_INCLUDE_STOMP_MOVEIT_COST_FUNCTIONS_TOOL_PATH_POSE_H_
 ...
 */
class ToolPathPose: public StompCostFunction
{
public:
  ToolPathPose();
  virtual ~ToolPathPose();
   ...
   * @param costs             vector containing the state costs per timestep.  [num_parameters x 1]
...
protected:
  ...
  std::vector<Eigen::Affine3d> tool_traj_pose;
  ...
  Eigen::Vector3d orientation_cost_weight_;                /**< @brief factor multiplied to the scaled position error **/ 


  // partial results variables
  Eigen::MatrixXd joint_pose_;
  Eigen::VectorXd last_joint_pose_;
  Eigen::Affine3d tool_pose_;
